...
describe('login', function () {
  ...
  describe('XML-RPC', function () {
    const data = {
      ...
      last_exec_event: 0,
      options: ['buddy-list', 'inventory-root', 'inventory-skeleton'],
      agree_to_tos: 'true',
      read_critical: 'true',
    };
    let methodCall;
      ...
      it("should create a fake MAC-Address if the user doesn't have one", function (done) {
        ...
        request(server)
          ...
          .expect(function () {
            ...
            assert.ok(
              /(?:[a-fA-F\d]{1,2}:){5}[a-fA-F\d]{1,2}/i.test(
      ...
      it('should validate the stored mac address', function (done) {
        ...
        request(server)
          ...
          .expect(function () {
            ...
            assert.ok(
              usersDbInsert.firstCall.firstArg.mac !== wrongMac,
              'mac was updated'
            );
            assert.strictEqual(
              usersDbInsert.firstCall.firstArg.mac,
    ...
    describe('with logged in user', function () {
      ...
      it('should use the existing fake MAC-Address', function (done) {
        ...
        request(server)
          ...
          .expect(function () {
            assert.ok(
              fetch.firstCall.args[1].body.includes(
                '<key>id0</key><string>db:35:c3:48:d8:4b</string>'
              ),
              'mac in request body'
            );
            assert.ok(
              fetch.firstCall.args[1].body.includes(
                '<key>mac</key><string>db:35:c3:48:d8:4b</string>'
              ),
              'mac in request body'
            );
            assert.ok(!usersDbInsert.called, 'update user doc');
