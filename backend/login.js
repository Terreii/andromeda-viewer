...
async function handleLogin(req, res) {
  try {
    ...
    const loginURL =
      reqLoginURL && reqLoginURL.length > 0
        ? new URL(reqLoginURL)
...
function handleXmlRpc(app, res, loginURL, reqData) {
  const xmlrpcClient =
    loginURL.protocol == null || loginURL.protocol === 'https:'
  ...
  switch (typeof value) {
    ...
    case 'object':
      if (Array.isArray(value)) {
        ...
        const mapBody = Object.keys(value).map(
          (key) => `<key>${key}</key>${stringifyLLSD(value[key])}`
        );
...
async function getMacAddress(req, res, next) {
  ...
  if (
    LOGIN_USER_ID_HEADER in req.headers &&
    req.headers[LOGIN_USER_ID_HEADER].length > 0
  ) {
